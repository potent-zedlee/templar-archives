 ...
 * 데이터베이스 상태 롤백
 */
export async function rollbackSubmissionStatus(
  supabase: any,
  context: RollbackContext,
  error: Error
): Promise<void> {
  try {
    console.log(`[rollback] Rolling back submission ${context.submissionId} to status: ${context.previousStatus}`)
    ...
    console.log(`[rollback] Successfully rolled back submission ${context.submissionId}`)
  } catch (rollbackError) {
    console.error(`[rollback] Failed to rollback submission ${context.submissionId}:`, rollbackError)
    // Rollback 실패는 로깅만 하고 에러를 throw하지 않음
  }
}
 ...
 * 파이프라인 실행 with 자동 롤백
 */
export async function withPipelineRollback<T>(
  supabase: any,
  submissionId: string,
  previousStatus: string,
  fn: () => Promise<T>
  ...
  try {
    ...
    await rollbackSubmissionStatus(supabase, context, err)
    throw err
  }
}
