...
export function TournamentCard({ tournament, onClick }: TournamentCardProps) {
  // 이벤트, 스트림, 핸드 카운트 계산
  const eventCount = tournament.events?.length || tournament.eventCount || 0
  const streamCount = tournament.events?.reduce(
    (total, event) => total + (event.streams?.length || 0),
    0
  ) || tournament.streamCount || 0
          ...
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="relative flex-shrink-0">
              <Avatar className="w-16 h-16 rounded-lg border-2 border-muted">
                <AvatarImage
                  src={tournament.categoryLogoUrl || `/logos/${tournament.category}.png`}
                  alt={tournament.name}
                />
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>
                  ...
                  {tournament.endDate && tournament.endDate !== tournament.startDate && (
                    <> - {new Date(tournament.endDate).toLocaleDateString('ko-KR', {
                      month: 'short',
                      day: 'numeric'
                    })}</>
                  )}
