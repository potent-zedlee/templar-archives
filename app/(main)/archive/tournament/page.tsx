...
/**
 * Archive Tournament Page - Phase 42 Refactor
 *
...
import { Trophy, Search, Plus, Filter, ArrowUpDown } from "lucide-react"
import { useAuth } from "@/components/auth-provider"
import { ErrorBoundary } from "@/components/error-boundary"
import { PageTransition } from "@/components/page-transition"
import { ArchiveAccordion } from "../_components/ArchiveAccordion"
import { useTournamentsQuery, useHandsQuery } from "@/lib/queries/archive-queries"
...
export default function ArchiveTournamentPage() {
  ...
  // ============================================================
  const { data: tournaments = [], isLoading: tournamentsLoading } = useTournamentsQuery("tournament")
  ...
  const handsMap = useMemo(() => {
    ...
    allStreams.forEach((stream) => {
      // 각 stream의 hands를 가져와서 Map에 저장
      // 실제로는 useHandsQuery를 각 stream마다 호출하는 대신
      // 미리 모든 hands를 가져온 후 stream별로 그룹핑하는 것이 효율적
      // 여기서는 간단히 빈 배열로 초기화
  ...
  const handleAddTournament = () => {
    // TODO: Open Tournament Dialog
    console.log("Add Tournament")
  }

  const handleAddEvent = (tournamentId: string) => {
    console.log("Add Event to tournament:", tournamentId)
  }
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-4xl font-bold text-gold-400">Tournament Archive</h1>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  {filteredTournaments.length} tournaments • {allStreams.length} streams
                </p>
              </div>
            </div>
